...
////GAMEPAD////
typedef DWORD WINAPI win_xinput_get_state (DWORD dwUserIndex , XINPUT_STATE *pState); // Gamepad Controller_Input
typedef DWORD WINAPI win_xinput_set_state (DWORD dwUserIndex , XINPUT_VIBRATION *pVibration); // set vibration to our gamepad
...
typedef XINPUT_SET_STATE (win_xinput_set_state);
typedef XINPUT_GET_STATE (win_xinput_get_state);


// Stubs functions returns errors about gamepad connection and stop her
...
global_variable win_xinput_get_state *XInputGetState_= XInput_Get_State_Stub ; // Gamepad Controller
...
int WINAPI // try to rename CALLBACK
				...
				int nCmdShow)
	{
		...
		if (RegisterClassA(&WindowClass)) // is Val is 0 , we continue a project . Another way , we exist
		{
			HWND Window = CreateWindowExA(0, WindowClass.lpszClassName, "Hero",
